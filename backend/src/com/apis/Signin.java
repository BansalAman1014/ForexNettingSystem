	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if(userUtil.validateUser(email, password)) {
				...
				// else error response will be sent which will state user didn't validated with given credentials
			...
			String errorJson = RequestResponseUtility.logAndBuildJsonOfSystemException(this.getClass().getName(), "doPost", e);
			RequestResponseUtility.buildErrorResponse(request, response, errorJson);
		}
	}
